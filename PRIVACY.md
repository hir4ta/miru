...
- **jsDelivr CDN.** The Markdown browser preview's mermaid script and svg-pan-zoom (used to power click-to-zoom on mermaid diagrams) are served by `cdn.jsdelivr.net` and are fetched only when the Markdown contains a `mermaid` code block. See [jsDelivr's privacy policy](https://www.jsdelivr.com/terms/privacy-policy-jsdelivr-net).
