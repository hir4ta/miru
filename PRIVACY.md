...
| `b` (browser preview) on a Markdown file that contains a `mermaid` code block, once the page loads | `cdn.jsdelivr.net` / `fonts.googleapis.com` / `fonts.gstatic.com` | Fetched by your browser for mermaid.js and the Caveat hand-drawn font | Don't press `b`, omit mermaid blocks, or block the hosts |

The Markdown browser preview embeds `<script src="https://cdn.jsdelivr.net/npm/mermaid@11/dist/mermaid.esm.min.mjs">` and the Google Fonts stylesheet (`https://fonts.googleapis.com/css2?family=Caveat...`, which in turn fetches the woff2 from `fonts.gstatic.com`) **only when the rendered Markdown contains a `mermaid` code block**; Markdown previews without a mermaid block embed neither and stay fully local. Source-file previews (non-Markdown files opened via `b`) inline chroma-generated styles and likewise fetch nothing. **The third-party requests above are initiated by your browser, not by miru.** To avoid them, do not press `b` on Markdown with mermaid blocks, or use a browser with the resources blocked (uBlock Origin, NoScript).
...
- **Google Fonts.** The Markdown browser preview's hand-drawn diagram font (Caveat) is served by `fonts.googleapis.com` and `fonts.gstatic.com` and is fetched only when the Markdown contains a `mermaid` code block. See [Google's privacy policy](https://policies.google.com/privacy) and the [Google Fonts FAQ](https://developers.google.com/fonts/faq#what_does_using_the_google_fonts_api_mean_for_the_privacy_of_my_users).
